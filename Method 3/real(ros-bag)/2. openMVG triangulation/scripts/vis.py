...
def create_json():
    ...
    my_file = open(os.path.join(sys.path[0], "../data/known/images.txt"), "r") # IMAGE_ID, QW, QX, $# reading the file
...
1 PINHOLE 1920 1080 1866.6666 1575.0 960.0 540.0"""
    ...
    print("""{
        ...
        "root_path": \"""", os.path.join(sys.path[0], "../data/images"),"""\",
        "views": [ 
        """, sep='', end='', file=f)
...
pFeatures = subprocess.Popen( [os.path.join(OPENMVG_SFM_BIN, "openMVG_main_openMVG2WebGL"),  "-i", "../sfm_data.json", "-o", "./"] )
pFeatures.wait()
...
# Replace the target string
with open(os.path.join(sys.path[0], 'model/model.js'), 'r') as file :
  filedata = file.read()
...
with open(os.path.join(sys.path[0], 'model/model.js'), 'w') as file:
  file.write(filedata)
file.close()
